<section class="cabecalho-pagina">
    <div class="botoes">
        ...
        <div class="pull-right">
            {if !$remover }
                <a class="btn btn-primary" href="/admix/produtos/inserir/?url_retorno={$url_retorno}"><i class="icon-plus"></i> <span class="hide-mobile">Cadastrar</span></a>
                <a class="btn btn-success procurar" href="#"><i class="icon-search"></i> <span class="hide-mobile">Procurar</span></a>
                <a class="btn btn-warning imprimir" href="/admix/produtos/?print=true&{$smarty.server.QUERY_STRING}" target="_blank"><i class="icon-print"></i> Imprimir</a>
            ...
            {else}
                ...
                <a class="btn" href="/admix/produtos/{$url_retorno|urldecode}"><i class="icon-arrow-left"></i> <span class="hide-mobile">Voltar</span></a>
            {/if}		
        </div>
    </div>
</section>
                    ...
                    <div class="span2">
                        ...
                        <select name="categoria_id" id="categoria_id" data-placeholder="Escolha uma opção" class="span12 chosen">
                            ...
                            {foreach $categorias as $categoria}
                                <option value="{$categoria.id}" {if {$categoria.id} == $b.id} selected="selected" {/if}>{$categoria.descricao}</option>
                            {/foreach}
                        </select>
                    </div>
                    <div class="span2">
                        ...
                        <thead>
                            <tr>
                                ...
                                <th style="text-align: center">
                                    {adm_table_title titulo='Preço (R$)' campo='preco' url_base=$base_url_order ord_campo=$ord_campo ord_tipo=$ord_tipo remover=$remover}
                                </th>
                                <th style="text-align: center">
                                    Imagem
                                </th>
                                ...
                                <tr>
                                    ...
                                    {if !$remover}
                                        <td data-title="Remover / Editar" class="no-print sem-clique" style="vertical-align: middle">
                                            <a href="/admix/produtos/alterar/{$v.id}/?url_retorno={$url_retorno}" rel="tooltip" title="Alterar" class="btn-action btn-success"><i class="icon-pencil default"></i></a>
                                            <a href="/admix/produtos/remover/{$v.id}/?url_retorno={$url_retorno}&ord_campo={$ord_campo}&ord_tipo={$ord_tipo}" rel="tooltip" title="Remover" class="btn-action btn-danger"><i class="icon-remove default"></i></a>
                                        </td>
                                    {/if}
                                </tr>
                            {/foreach}
