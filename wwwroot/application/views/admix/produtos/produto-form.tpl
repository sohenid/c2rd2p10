<form method="post" action="{$action}" class="form-horizontal form-interno validar" enctype="multipart/form-data">
    ...
    <fieldset>
        ...
        <div class="control-group {if $e.categoria_id}error{/if}">
                ...
                <div class="span2 no-margin">
                    <select name="categoria_id" id="categoria_Id" data-placeholder="Escolha uma opção" class="span2 chosen" required="required" title="Categoria">
                        ...
                        {foreach $categorias as $categoria}
                            <option value="{$categoria.id}" {if {$categoria.id} == $v.categoria_id} selected="selected" {/if}>{$categoria.descricao}</option>
                        {/foreach}
                    </select>
                    {if $e.categoria_id}<span class="help-inline">{$e.categoria_id}</span>{/if}
                </div>
            </div>
        </div>
        ...
        <div class="control-group {if $e.preco}error{/if}">
            <label class="control-label" for="preco">Preço</label>
            <div class="controls">
                <input id="preco" name="preco" class="span6 mask-dinheiro" type="text" value="{if $v.preco} {$v.preco|number_format:2:',':'.'}{/if}" required="required" title="Preço">
                {if $e.preco}<span class="help-inline">{$e.preco}</span>{/if}
            </div>
        </div>
            ...
            <div class="controls">
                ...
                <span class="help-inline input-file-actions">
                    <a href="#" class="fake-link btn-action btn-info" rel="tooltip" title="Arquivos .swf .jpg .png"><i class="icon-question-sign default"></i></a>
                        ...
                        <a href="javascript:if(confirm('Deseja realmente remover este arquivo?')){ location.href='/admix/produtos/removerArquivo{$v.imagem}/?url_retorno={$smarty.server['REQUEST_URI']|urlencode|urlencode|urlencode}'}" rel="tooltip" title="Remover" class="btn-action btn-danger"><i class="icon-remove default"></i></a>
                        {/if}
                </span>
            </div>
        </div>
