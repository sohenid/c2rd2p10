...
class Categorias extends Admix_Controller {
    ...
    public function index() {
        ...
        $b = array('id', 'descricao', 'imagem', 'status', 'data_insert', 'data_update');
        foreach ($b as $k => $v) {
        ...
        $d = $this->categorias->getCategorias($q, $ord, $per_page, $offset);
        ...
        $this->setPagina($this->router->fetch_class() . '/categorias.tpl', $dados);
    }
    ...
    public function inserir() {
        ...
        $campos = array('id', 'descricao', 'imagem', 'status', 'data_insert', 'data_update');
        foreach ($campos as $k) {
        ...
        $this->setPagina($this->router->fetch_class() . '/categoria-form.tpl', $dados);
    }
    ...
    public function inserirPost() {
        ...
        $data['data_insert'] = date('Y-m-d H:i:s');
        ...
        $upload_errors = FALSE;
        $upload = mixdUpload('imagem', '/media/categorias/', $data['descricao'], NULL, FALSE);
        if ($upload) {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $result = $this->categorias->inserir($data);
    ...
    public function alterar($id = NULL) {
        ...
        $d = $this->categorias->getCategorias($q);
        $resposta = $d['result'][0];
        $errors = $this->session->flashdata('form_errors') ? $this->session->flashdata('form_errors') : array();
    ...
    public function alterarPost() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $result = $this->categorias->alterar($data);
            if (!(isset($result['error']))) {
    ...
    public function remover($id = NULL) {
        ...
        $q = array();
        $q['where_in']['id'] = $id;
        $d = $this->categorias->getCategorias($q, $ord);
        if (!$d['result']) {
    ...
    public function removerPost() {
        ...
        $result = $this->categorias->remover($data);
        if (!(isset($result['error']))) {
    ...
    public function removerArquivo() {
        ...
        $file = str_replace('admix/' . $this->router->fetch_class() . '/' . $this->router->fetch_method() . '/', '', $url);
        $file = $this->security->sanitize_filename($file, TRUE);
        $result = $this->categorias->removerArquivo($file);
        if (!(isset($result['error']))) {
            $remover = mixdUploadRemover($file);
